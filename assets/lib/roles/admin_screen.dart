...
// SignOut kafolatli ishlashi uchun
import 'payment_review_screen.dart';
import 'room_assignment_api_screen.dart';
import '../modules/models/user_model.dart';
import '../modules/talabalar/talabalar_list.dart';
import '../modules/xonalar/yotoqxona_turlari_screen.dart';
...
// Ijtimoiy imtiyoz hujjatlari sahifasi
...
class _AdminScreenState extends State<AdminScreen> {
  ...
  bool get _canEditRooms => widget.user.hasPermission('canEditRooms');
  bool get _canExportExcel => widget.user.hasPermission('canExportExcel');

  final List<_NavItem> _navItems = const [
    _NavItem('Dashboard', Icons.space_dashboard_rounded,
        Icons.space_dashboard_outlined, _C.violet, 'accessDashboard'),
    _NavItem('Umumiy ro\'yxat', Icons.public_rounded, Icons.public_outlined,
        _C.mint, 'accessUmumiyRoyxat'),
    _NavItem('Talabalar va Hodimlar', Icons.groups_rounded,
        Icons.groups_outlined, _C.teal, 'accessTalabalar'),
    _NavItem('Xonalar', Icons.apartment_rounded, Icons.apartment_outlined,
        _C.orange, 'accessXonalar'),
    _NavItem('Xonaga biriktirish', Icons.meeting_room_rounded,
        Icons.meeting_room_outlined, _C.mint, 'accessXonaBiriktirish'),
    ...
    _NavItem('Ijtimoiy imtiyozlar', Icons.volunteer_activism_rounded,
        Icons.volunteer_activism_outlined, _C.pink, 'accessImtiyozlar'),
  ...
  bool _hasAccess(_NavItem item) =>
      widget.user.hasPermission(item.permissionKey);
  ...
  void initState() {
    ...
    _tabs.addAll([
      ...
      YotoqxonaTurlariScreen(
        isAdmin: true,
        canEdit: true,
        genderHostel: widget.user.hostel ?? 'boys',
      ),
      // ✅ Laravel backend orqali ishlaydigan xonaga biriktirish oqimi
      // (yotoqxona_backend). "Xonalar" tabidagi eski Firestore-asosli
      // ekrandan farqli — bu yerda talaba/xona/biriktirish ma'lumotlari
      // Postgres bazasidan (GET /rooms, /students, /room-assignments)
      // olinadi.
      const RoomAssignmentApiScreen(),
    ...
    if (!_hasAccess(_navItems[_selectedIndex])) {
      final firstAllowed = _navItems.indexWhere((item) => _hasAccess(item));
      if (firstAllowed != -1) _selectedIndex = firstAllowed;
    }
  }
  ...
  void _logout() async {
    try {
      // ✅ Laravel backend: Sanctum tokenini serverda bekor qilamiz
      // va lokal saqlangan tokenni tozalaymiz.
      try {
          ...
          onTap: () {
            if (!hasAccess) {
              ...
              ScaffoldMessenger.of(context).showSnackBar(
                SnackBar(
                  content:
                      Text("\"${item.label}\" bo'limi uchun ruxsatingiz yo'q."),
...
class _NavItem {
  ...
  final IconData icon;
  final Color color;
  // Ushbu bo'limga kirish uchun talab qilinadigan huquq kaliti
  // (UserModel.hasPermission() bilan tekshiriladi, masalan 'accessXonalar').
        ...
        actions: [
          ...
          ElevatedButton.icon(
            ...
            onPressed: () async {
              ...
              await _deleteUser(userId, fullName);
            },
          ),
        ],
      ),
                          ...
                          Builder(builder: (context) {
                            ...
                            return Padding(
                              ...
                              child: GestureDetector(
                                onTap: () =>
                                    _confirmDelete(user['id'], fullName),
                                child: Container(
                                  width: 34,
                                  height: 34,
                                  decoration: BoxDecoration(
              ...
              SingleChildScrollView(
                ...
                child: Column(
                  children: [
                    ...
                    // bilan berilmaydi.
                    if (_isSuperAdmin || _canExportExcel) ...[
                      const SizedBox(height: 14),
                      _SettingsCard(
