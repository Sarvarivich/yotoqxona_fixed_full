...
class XonalarList extends StatefulWidget {
  final bool isAdmin;
  final String hostel;
  // Xonalarni qo'shish/tahrirlash (holatini o'zgartirish) huquqi.
  // Berilmasa, isAdmin bilan bir xil bo'ladi (eski chaqiruvlar bilan
  // moslik uchun — masalan mudir har doim to'liq tahrirlash huquqiga
  // ega). Bu ajratish superAdmin va 'canEditRooms' huquqi berilmagan
  // cheklangan "admin" o'rtasida farq qilish uchun kerak: ikkalasi ham
  // ro'yxatni ko'radi, lekin faqat huquqi borlar tahrirlay oladi.
  final bool? canEdit;

  /// 5 ta yotoqxona turidan qaysi biri ochilganini bildiradi.
...
class _XonalarListState extends State<XonalarList> {
                                  ...
                                  try {
                                    ...
                                    final duplicateInSameHostel =
                                      ...
                                      final normalized = docHostel.isEmpty
                                          ? 'boys'
                                          : docHostel;
                                      final rawType = (data['hostelType'] ?? '')
                                          .toString()
                                          .trim()
                                          .toLowerCase();
                                      final normalizedType = rawType.isEmpty
                                          ? 'university'
                                          : _normalizeHostelType(rawType);
                                      return normalized == _selectedHostel &&
                                          normalizedType == _hostelType;
                                    });
  ...
  bool _matchesRentalStudent(Map<String, dynamic> data) {
    if (_searchQuery.trim().isEmpty) return true;
    final q = _searchQuery.trim().toLowerCase();

    final values = <String>[
      (data['fullName'] ?? data['name'] ?? data['fio'] ?? '').toString(),
      (data['phone'] ?? data['telefon'] ?? '').toString(),
      (data['jshshir'] ?? data['JSHSHIR'] ?? '').toString(),
      (data['studentId'] ?? data['student_id'] ?? '').toString(),
      (data['faculty'] ?? data['fakultet'] ?? '').toString(),
      (data['course'] ?? data['kurs'] ?? '').toString(),
      (data['region'] ?? data['viloyat'] ?? '').toString(),
      (data['district'] ?? data['tuman'] ?? '').toString(),
    ];
  ...
  String _studentName(Map<String, dynamic> data) {
    final name = (data['fullName'] ??
        ...
        .toString()
        .trim();
    return name.isEmpty ? 'Noma\'lum talaba' : name;
  }

  Widget _buildRentalStudentCard(BuildContext context, DocumentSnapshot doc) {
    final data = doc.data() as Map<String, dynamic>;
    final name = _studentName(data);
    final phone = (data['phone'] ?? data['telefon'] ?? '—').toString();
    final studentId =
        (data['studentId'] ?? data['student_id'] ?? data['jshshir'] ?? '—')
            .toString();
    final faculty = (data['faculty'] ?? data['fakultet'] ?? '—').toString();
    final course = (data['course'] ?? data['kurs'] ?? '—').toString();
    final region = (data['region'] ?? data['viloyat'] ?? '—').toString();
    final social = (data['socialStatus'] ??
            data['social_status'] ??
            data['ijtimoiyHolat'] ??
            data['ijtimoiy_holat'] ??
            '')
              ...
              content: SingleChildScrollView(
                child: Column(
                  ...
                  children: [
                    Text('Telefon: $phone'),
                    const SizedBox(height: 8),
                    Text('Student ID / JSHSHIR: $studentId'),
                    const SizedBox(height: 8),
                    Text('Fakultet: $faculty'),
                    const SizedBox(height: 8),
                    Text('Kurs: $course'),
                    const SizedBox(height: 8),
                    Text('Viloyat: $region'),
                    if (social.isNotEmpty) ...[
                      const SizedBox(height: 8),
                      Text('Ijtimoiy holat: $social'),
                    ],
                    const SizedBox(height: 12),
                    const Text(
                      'Yotoqxona turi: Ijara uchun ajratilgan',
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    Container(
                      ...
                      decoration: BoxDecoration(
                        color: _LC.teal.withOpacity(.12),
                        borderRadius: BorderRadius.circular(12),
                      ),
                      child: const Text(
                        'Ijara uchun ajratilgan',
                        style: TextStyle(
                          color: _LC.teal,
                          fontSize: 11,
                          fontWeight: FontWeight.w800,
                    ...
                    Wrap(
                      ...
                      runSpacing: 8,
                      children: [
                        _rentalInfo(Icons.phone_rounded, phone),
                        _rentalInfo(Icons.badge_outlined, studentId),
                        _rentalInfo(
                            Icons.school_outlined, '$faculty / $course'),
                        _rentalInfo(Icons.map_outlined, region),
                      ],
                    ),
                  ],
                ),
  ...
  Widget _rentalInfo(IconData icon, String value) {
    return ConstrainedBox(
      constraints: const BoxConstraints(maxWidth: 360),
      child: Row(
        mainAxisSize: MainAxisSize.min,
        children: [
          Icon(icon, size: 17, color: _LC.muted),
          const SizedBox(width: 6),
          Flexible(
            child: Text(
              value,
  ...
  Widget build(BuildContext context) {
    // Ijara bo'limi xona boshqaruvi emas: bu yerda faqat ijaraga
    // biriktirilgan talabalar ro'yxati va qidiruv ko'rsatiladi.
            ...
            Padding(
              ...
              child: Container(
                ...
                child: TextField(
                  ...
                  decoration: InputDecoration(
                    hintText:
                        'FIO, telefon, JSHSHIR yoki fakultet bo‘yicha qidirish...',
                    hintStyle:
              ...
              child: StreamBuilder<QuerySnapshot>(
                ...
                builder: (context, snapshot) {
                  ...
                  final students = snapshot.data!.docs.where((doc) {
                    final data = doc.data() as Map<String, dynamic>;
                    final role = (data['role'] ?? 'talaba')
                        .toString()
                        .trim()
                        .toLowerCase();
                    if (role != 'talaba' && role != 'student') {
                      return false;
                    }

                    final rawType = (data['hostelAssignmentType'] ??
                            data['hostelType'] ??
                            '')
                        .toString()
                        .trim();
                    ...
                    if (rawType.isEmpty ||
                        _normalizeHostelType(rawType) != 'rental') {
                      return false;
                    }

                    return _matchesRentalStudent(data);
                  }).toList();
                    ...
                    return Center(
                      child: Column(
                        ...
                        children: const [
                          ...
                          SizedBox(height: 12),
                          Text(
                            'Ijara uchun biriktirilgan talabalar topilmadi.',
                            textAlign: TextAlign.center,
                            style: TextStyle(
                              color: _LC.muted,
                              fontWeight: FontWeight.w600,
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: LayoutBuilder(
          builder: (context, constraints) {
            // Sarlavha barcha yotoqxona bo'limlarida bir xil:
            // "<Yotoqxona nomi>" — foydalanuvchi yuborgan dizayndagi kabi.
            return SizedBox(
              width: double.infinity,
              child: FittedBox(
        ...
        bottom: PreferredSize(
          // Sarlavha bilan "O'g'il bolalar / Qiz bolalar" tugmalari
          // orasida yetarli vertikal masofa qoldiramiz.
          preferredSize: const Size.fromHeight(132),
          child: Padding(
            padding: const EdgeInsets.fromLTRB(16, 1.5, 16, 1),
            child: Column(
              children: [
      ...
      body: StreamBuilder<QuerySnapshot>(
        ...
        builder: (context, snapshot) {
          ...
          final docs = snapshot.data!.docs.where((doc) {
            ...
            final rawType =
                (data['hostelType'] ?? '').toString().trim().toLowerCase();
            final normalizedType =
                rawType.isEmpty ? 'university' : _normalizeHostelType(rawType);
            ...
            return Center(
              child: Column(
                ...
                children: [
                  ...
                  const SizedBox(height: 12),
                  Text(
                      "$genderLabel — $_hostelTypeTitle bo‘limida hozircha xona yo‘q.",
                      textAlign: TextAlign.center,
                      style: const TextStyle(
                ...
                child: ConstrainedBox(
                  ...
                  child: GridView.builder(
                    ...
                    itemBuilder: (context, index) {
                      ...
                      return _buildRoomCard(context, room, _canEdit, docId);
                    },
                  ),
                ),
              );
