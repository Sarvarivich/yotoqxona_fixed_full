...
class _TalabaTolovlarScreenState extends State<TalabaTolovlarScreen> {
  ...
  bool _isDownloadingContract = false;

  Future<void> _downloadContract() async {
    final user = widget.user;
    if (user == null || !user.hasRoom || _isDownloadingContract) return;

    setState(() => _isDownloadingContract = true);
    try {
      final bytes = await ApiService().downloadContractBytes();
final result = await openOrDownloadFile(
  bytes,
  'Yotoqxona_Shartnomasi_${user.fullName.replaceAll(' ', '_')}.doc',
);
      ...
      if (mounted) setState(() => _isDownloadingContract = false);
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    if (user == null || !user.hasRoom) {
      return const Scaffold(
        ...
        body: Center(
          child: Text(
            "Yotoqxonaga biriktirilmaguningizcha to‘lovlar bo‘limi ko‘rinmaydi.",
            ...
            Padding(
              ...
              child: Row(
                children: [
                  ...
                  FutureBuilder<bool>(
                    future: ApiService().isContractAvailable(),
                    builder: (context, snap) {
                      final available = snap.data == true;
                      return FilledButton.icon(
                        onPressed: (available && !_isDownloadingContract)
                            ? _downloadContract
                            : null,
                        icon: _isDownloadingContract
                            ? const SizedBox(
                                width: 16,
                                height: 16,
