...
class _TalabaProfileScreenState extends State<TalabaProfileScreen> {
  ...
  void initState() {
    super.initState();
    _user = widget.user;
    _listenToStudentProfile();
  }

  void _goTo(int index) {
    // To'lovlar faqat jismoniy yotoqxona + xona biriktirilganda ochiladi.
    if (index == 2 && !_user.hasRoom) return;
    setState(() => _tab = index);
  }
  ...
  void _listenToStudentProfile() {
    ...
    _userSubscription = FirebaseFirestore.instance
      ...
      if (data == null) return;
      final updated = UserModel.fromJson({...data, 'id': snap.id});
...
class _TalabaDrawer extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final items = <(int, IconData, String)>[
      (0, Icons.home_rounded, 'Bosh'),
      (1, Icons.apartment_rounded, 'Yotoqxona'),
      if (hasRoom) (2, Icons.receipt_long_rounded, "To'lovlar"),
      (3, Icons.person_rounded, 'Profil'),
      (4, Icons.forum_rounded, 'Murojaat'),
    ];
...
String _applicationStepTitle(UserModel user) {
  switch (user.applicationStep) {
    case 1:
      return "Arizani to'ldiring";
    case 2:
      return "Arizangiz ko'rib chiqilmoqda";
    case 3:
      return "Siz uchun yotoqxona ajratildi";
    case 4:
...
String? _applicationStepMessage(UserModel user) {
  if (user.applicationStep == 3) {
    if (user.isRentalAssignment) {
      return "To'liq ma'lumot olish uchun Yoshlar bilan ishlash departamentiga murojaat qiling.";
    }
    return user.assignmentMessage ??
        '${user.hostelDisplayName}ga biriktirildingiz.';
  }
  if (user.applicationStep == 4) {
    return "Hozirgi to'lovlar bo'limidan shartnomani yuklab oling va chekni moliyaga yuboring.";
  }
  if (user.applicationStep == 5) {
    return "Moliya bo'limi to'lovingizni tasdiqladi.";
  }
  return null;
}
            ...
            SliverPadding(
              ...
              sliver: SliverList(
                delegate: SliverChildListDelegate([
                  ...
                  const SizedBox(height: 16),
                  ApplicationStepper(
                    activeStep: user.applicationStep,
                    subtitle: _applicationStepTitle(user),
                    hostelMessage: _applicationStepMessage(user),
                  ),
